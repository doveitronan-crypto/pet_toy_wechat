...
App({
  onLaunch: function () {
    // Check if we have storage data, otherwise initialize defaults
    ...
    const activePet = wx.getStorageSync('activePet') || pets[0];
    const isMatched = wx.getStorageSync('isMatched') !== undefined ? wx.getStorageSync('isMatched') : true;
    const wishlist = wx.getStorageSync('wishlist') || [];
    const wishlistNotes = wx.getStorageSync('wishlistNotes') || {};
    const blindBoxHistory = wx.getStorageSync('blindBoxHistory') || [];

    this.globalData.pets = pets;
    this.globalData.activePet = activePet;
    this.globalData.isMatched = isMatched;
    this.globalData.wishlist = wishlist;
    this.globalData.wishlistNotes = wishlistNotes;
    this.globalData.blindBoxHistory = blindBoxHistory;
  },
  ...
  // State modifiers
  addPet: function (pet) {
    const updated = [...this.globalData.pets, pet];
    this.saveToStorage('pets', updated);
  },

  deletePet: function (petId) {
    if (this.globalData.pets.length <= 1) return;
    const remaining = this.globalData.pets.filter(p => p.id !== petId);
    this.saveToStorage('pets', remaining);
    
    // Default always enable matching on remaining pet
    this.saveToStorage('isMatched', true);
    
    if (this.globalData.activePet.id === petId) {
      this.saveToStorage('activePet', remaining[0]);
    }
  },

  setActivePet: function (pet) {
    this.saveToStorage('activePet', pet);
  },

  setIsMatched: function (matched) {
    this.saveToStorage('isMatched', matched);
  },

  toggleWishlist: function (toy) {
    const exists = this.globalData.wishlist.some(t => t.id === toy.id);
    let updated;
    if (exists) {
      updated = this.globalData.wishlist.filter(t => t.id !== toy.id);
    } else {
      updated = [...this.globalData.wishlist, toy];
    }
    this.saveToStorage('wishlist', updated);
    return !exists;
  },

  saveWishlistNote: function (toyId, note) {
    const notes = { ...this.globalData.wishlistNotes };
    notes[toyId] = note;
    this.saveToStorage('wishlistNotes', notes);
  },

  addHistory: function (item) {
    const updated = [item, ...this.globalData.blindBoxHistory];
    this.saveToStorage('blindBoxHistory', updated);
  },

  deleteHistoryItem: function (id) {
    const updated = this.globalData.blindBoxHistory.filter(item => item.id !== id);
    this.saveToStorage('blindBoxHistory', updated);
  },

  clearHistory: function () {
    this.saveToStorage('blindBoxHistory', []);
  }
});
